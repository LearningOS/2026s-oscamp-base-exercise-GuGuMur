...
unsafe impl GlobalAlloc for FreeListAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        while !curr.is_null() {
            ...
            if curr_addr % align == 0 && unsafe { (*curr).size } >= size {
        ...
        loop {
            let alloc_start = (prev_next + align - 1) & !(align - 1); // Align up
            ...
            match self.bump_next.compare_exchange(
                ...
                Ok(_) => return alloc_start as *mut u8, // Successfully allocated
                Err(next) => prev_next = next,          // Retry with updated bump_next
            }
        }
    }
